...
fun CategoryListScreen(
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            if (uiState.categories.isEmpty()) {
                Column(
                    ...
                    Text("Belum ada kategori", style = MaterialTheme.typography.bodyMedium, color = TextMuted)
                }
            } else {
                LazyColumn(
    ...
    when (val dialog = uiState.dialog) {
        is CategoryDialog.Add -> AddEditCategoryDialog(
            title = "Tambah Kategori",
            initialName = "",
            onConfirm = { viewModel.addCategory(it) },
            onDismiss = viewModel::dismissDialog,
        )
        is CategoryDialog.Edit -> AddEditCategoryDialog(
            title = "Edit Kategori",
            initialName = dialog.category.name,
            onConfirm = { viewModel.updateCategory(dialog.category, it) },
            onDismiss = viewModel::dismissDialog,
        )
        is CategoryDialog.Delete -> DeleteCategoryDialog(
            category = dialog.category,
            onConfirm = { viewModel.deleteCategory(dialog.category) },
            onDismiss = viewModel::dismissDialog,
        )
        is CategoryDialog.None -> Unit
    }
}
    ...
    AlertDialog(
        ...
        text = {
            OutlinedTextField(
                ...
                label = { Text("Nama Kategori", style = MaterialTheme.typography.labelMedium) },
                singleLine = true,
...
@Composable
private fun DeleteCategoryDialog(category: Category, onConfirm: () -> Unit, onDismiss: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("Hapus Kategori?", style = MaterialTheme.typography.titleLarge, color = Ink) },
        text = {
            Text(
                text = "Kategori \"${category.name}\" akan dihapus. Tindakan ini tidak dapat dibatalkan.",
